...
if [ -f "$STATE_DIR/baseline-committed" ]; then
    ...
    if git rev-parse --git-dir > /dev/null 2>&1; then
        ...
        if [ -d "$STANDALONE_PROJECT" ] && [ "$(ls -A $STANDALONE_PROJECT 2>/dev/null)" ]; then
            log_info "Adding storefront-next to git..."

            # Add and commit the baseline (allow commit to fail if no changes)
...
- Note: node_modules is symlinked to /tmp and excluded by .gitignore" 2>&1 | tee -a "$LOG_FILE"; then
                    ...
                    # Commit may fail if no changes (already committed), which is fine
                    if git diff --cached --quiet; then
                        log_success "Storefront-next already in git (no changes to commit)"
                    else
        ...
        log_warning "/workspace is not a git repository, skipping git baseline"
...
if [ -f "/workspace/.claude-session-id" ]; then
    ...
    if [ -n "$SESSION_ID" ]; then
        ...
        if [ $CLAUDE_EXIT_CODE -ne 0 ]; then
            if grep -qi "Session not found\|No such session\|Invalid session\|No conversation found\|session.*expired\|not.*found" /workspace/claude-output.log 2>/dev/null; then
